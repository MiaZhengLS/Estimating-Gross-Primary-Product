...
from fttransformer.models.tabtransformer import TransformerBlock
from tensorflow.keras.layers import (
    Dense,
    Flatten,
)
import math as m
from fttransformer.models.embeddings import CEmbedding, NEmbedding
...
class FTTransformerEncoder(tf.keras.Model):
    def __init__(
        ...
        self.depth = depth
        self.heads = heads
        # d_hidden = int(embedding_dim * ffn_factor)    
...
class MLPBlock(tf.keras.layers.Layer):
    def __init__(self, unit_0, unit_1, out_dim, out_activation):
        super().__init__()
        self.layer_1 = tf.keras.layers.Dense(unit_0, activation='relu')
        self.layer_2 = tf.keras.layers.Dense(unit_1, activation='relu')
        self.layer_3 = tf.keras.layers.Dense(out_dim, activation=out_activation)
...
class DefaultBlock(tf.keras.layers.Layer):
    def __init__(self, embedding_dim, out_dim, out_activation):
        super().__init__()
        self.linear_1 = tf.keras.layers.LayerNormalization()
        self.linear_2 = Dense(embedding_dim//2, activation='relu')
        self.linear_3 = Dense(out_dim, activation=out_activation)
...
class FTTransformer(tf.keras.Model):
    def __init__(
        ...
        if head:
            ...
            self.head = DefaultBlock(embedding_dim, out_dim, out_activation)
